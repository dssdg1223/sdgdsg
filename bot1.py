...
print(decal)
print(f"seja bem vindo a minha ferramenta mn seja muito feliz aq {usuario} e sua idade é {idade} ela e compativel :) ")
...
WEBHOOK_URL = "https://discord.com/api/webhooks/1408684174470283316/MirgE3pX24XlNeq_6qfEv9CWskLP3Cmw0KtictQbyGt_NAqWA0d9PWBVeANfTAYwvwt-"
...
while True:
    ...
    if escolha == "1":
        ...
        url = f"https://api.trumail.io/v2/lookups/json?email={email}"
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            if data.get("deliverable") is False:
                print(f"O email {email} pode estar inválido ou vazado")
            else:
